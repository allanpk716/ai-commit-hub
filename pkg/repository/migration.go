...
// MigrateAddPushoverHookFields 添加 Pushover Hook 配置字段的迁移
func MigrateAddPushoverHookFields(db *gorm.DB) error {
	logger.Info("开始迁移：添加 Pushover Hook 配置字段")
	...
	// 将现有项目的通知模式设置为默认值
	result := db.Model(&models.GitProject{}).
		Where("notification_mode = '' OR notification_mode IS NULL").
		Update("notification_mode", "enabled")
