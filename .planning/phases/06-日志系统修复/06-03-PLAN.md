...
<objective>
修复 main.go 和 app.go 中的 logger 调用方法签名错误，确保日志调用符合 logrus/WQGroup logger 的 API 规范。

目的：修复核心启动代码中的错误日志调用，这是应用启动最先执行的代码，优先级最高。
输出：main.go 和 app.go 中所有 logger 调用使用正确的方法签名
</objective>
...
<verification>
1. 搜索 main.go 和 app.go 确认无错误调用：
   `grep -n 'logger\.\(Info\|Error\|Warn\|Debug\)([^,)]*,[^)]*)' main.go app.go`

2. 编译测试：
   `go build -o build/bin/ai-commit-hub.exe .`

3. 运行应用程序，观察启动日志输出格式是否正常
</verification>

<success_criteria>
- [ ] main.go 和 app.go 中 grep 搜索无结果
- [ ] 代码编译通过
- [ ] 日志输出格式符合标准
- [ ] 无编译错误或警告
</success_criteria>
