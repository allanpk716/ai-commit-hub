...
must_haves:
  truths:
    ...
    - "双击事件日志记录在应用日志中可见"
    - "构建成功,无编译错误"
    - "右键菜单显示基础菜单项 (03-02-PLAN 将添加'检查更新')"
  artifacts:
    - path: "go.mod"
      provides: "依赖声明,使用 lutischan-ferenc/systray v1.3.0 (按 TRAY-03 要求)"
      contains: "github.com/lutischan-ferenc/systray"
    - path: "app.go"
  ...
  key_links:
    - from: "app.go:onSystrayReady"
      to: "github.com/lutischan-ferenc/systray"
...
Purpose: 现有 getlantern/systray v1.2.2 不支持双击事件,需要升级到支持双击的 lutischan-ferenc/systray v1.3.0 (按 TRAY-03 要求),并实现双击托盘图标显示窗口的功能。
...
<task type="auto">
  <name>Task 1: 升级 systray 库到 lutischan-ferenc/systray v1.3.0 (按 TRAY-03 要求)</name>
  <files>go.mod</files>
  <action>
    1. 在 go.mod 中将依赖从 getlantern/systray v1.2.2 更换为 lutischan-ferenc/systray:
       - 移除: github.com/getlantern/systray v1.2.2
       - 添加: github.com/lutischan-ferenc/systray v1.3.0
    ...
    3. 验证依赖更新成功:
       ```bash
       go mod verify
       go list -m github.com/lutischan-ferenc/systray
       ```
  </action>
  <verify>go list -m github.com/lutischan-ferenc/systray 输出 v1.3.0</verify>
  <done>go.mod 显示 lutischan-ferenc/systray v1.3.0, go mod tidy 无错误</done>
</task>
...
<task type="auto">
  ...
  <action>
    1. 更新 import 语句 (line 21):
       ...
       改为: github.com/lutischan-ferenc/systray
...
<verification>
- [ ] go.mod 使用 lutischan-ferenc/systray v1.3.0
- [ ] app.go import 更新为 github.com/lutischan-ferenc/systray
